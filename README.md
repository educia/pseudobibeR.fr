...
El paquete expone dos APIs conceptuales:[cite:47][cite:48][cite:51]

- `biber()` → interfaz heredada del repositorio original (`pseudobibeR.fr`), con métodos S3 para objetos `spacyr_parsed` y `udpipe_connlu` y heurísticas `_fr`. Está disponible por compatibilidad.
- `biber_es()` → función principal para español; recibe la salida de `udpipe::udpipe_annotate()`, llama internamente a `parse_biber_features(..., language = "es")` y usa todos los bloques `_es` (tiempos, subordinación, pasivas, negación, modales, etc.).[cite:48][cite:51]

Para proyectos en español, la recomendación es usar directamente `biber_es()` sobre parseos UD de `spanish-gsd`.
...
Los diccionarios y listas de palabras para español viven en `data-raw/dict.yaml` y `data-raw/word_lists.yaml` y se empaquetan en los objetos `dict` y `word_lists` mediante el script de construcción ubicado en `data-raw/` (cuyo nombre conserva la convención del repositorio original; véase la sección [Nombres heredados del paquete francés](#nombres-heredados-del-paquete-francés)).[cite:33][cite:35]
...
Para regenerar los diccionarios tras editar los YAML, ejecuta desde la raíz del repositorio:

```r
source("data-raw/build_french_dictionaries.R")  # nombre heredado; genera dict/word_lists para español
```
...
Por compatibilidad histórica, algunos archivos y scripts mantienen la palabra `french` en el nombre: `data-raw/build_french_dictionaries.R`, `data-raw/french_examples.yaml`, `data-raw/french_raw_text.tsv`, los tests `test-french-*.R`, etc.[cite:52][cite:57]

El contenido de todos esos archivos está ya adaptado al español: los YAML contienen diccionarios y listas léxicas para español, y los scripts regeneran los objetos `dict` y `word_lists` que usa el extractor español.
Cualquier referencia a `french` en rutas o nombres de archivo es un vestigio del proyecto original y no implica que el paquete esté orientado al francés.
